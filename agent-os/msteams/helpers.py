...
async def send_teams_adaptive_card_async(
    service_url: str,
    conversation_id: str,
    card: dict,
    config: TeamsConfig,
    ...
    """Post an Adaptive Card as a Teams activity.

    Used for interactive prompts (Approve/Reject, follow-up choices). The
    Adaptive Card schema is v1.4 (widely supported in Teams). On
    Action.Submit the user's click POSTs back to our /messages webhook
    with `activity.value` set to the action's `data` payload — that is
    what the router's process_activity uses to resume paused runs.
    """
    activity: dict = {
        "type": "message",
        "attachments": [
...
def build_approval_card(tool_name: str, args_preview: str, run_id: str) -> dict:
    """Construct an Adaptive Card that asks the user to approve or reject a
    pending specialist call. The card's Submit actions carry a `data`
    payload the webhook uses to resume the paused run."""
    return {
        ...
        "body": [
            ...
            },
            {
                "type": "TextBlock",
                "text": (
                    f"Sentinel wants to run **{tool_name}**. This calls the "
                    "You.com Research API (10-60s, uses a research credit)."
                ),
                "wrap": True,
                "spacing": "Small",
            },
